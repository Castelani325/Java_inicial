...
public class Main { //sintaxe básica do JAVA
    public static void main(String[] args) { //Metodo de entrada da Classe (é o primeiro metodo que a JVM vai executar) //Deve ser executado sempre que criarmos um novo .java
...
//        System.out.println(minhaString);
        ...
        class Carro { //Classe carro
            ...
            //final significa que o atributo não pode ser alterado após ser atribuído à instância
            final String modelo; //Atributo modelo (variável)
            int quilometragem; //Atributo quilometragem (variável)
            final int revisao = 5000; //Atributo revisão (variável)
        ...
        //Pacotes : usado para organizar grupos de classes

        SerVivo humano = new SerVivo(30) {
            @Override
            public void respirar() {
                System.out.println("mimindo de novo...");
                // Trouxe apenas o metodo que eu não havia determinado conteúdo
                //O metodo dormir já havia conteúdo e nas abstratas esse metodo com conteúdo não pode ser puxado
            }
        };

        SerVivo EU = new Humano(42, "Fulano de Tal");
        ...
        //Quando uma classe filha herda os atributos e comportamentos de uma classe mãe

        //Tratamento de exceção : basicamente, usar try catch pra encontrar erros comuns no código

        Carro meuCarroDefeituoso = null;

        try {
          meuCarroDefeituoso.acelerar();
        }
        catch (NullPointerException exception){
            //NullPointerException → É o tipo da exceção que você esta tratando. -> tem varios tipos de exececoes
            //exception → É o nome da variável que irá guardar o objeto da exceção quando ela ocorrer.


                    System.out.println("Carro nulo, venda essa bagaça");
        }
